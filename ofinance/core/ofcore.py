...
class OfCore():
    ...
    def __init__(self, ticker) -> None:
        ...
        # Verifica se 'info' è vuoto
        if not stock.info or stock.info.get("currentPrice") is None:
            print("Errore: Ticker non valido o dati non disponibili.")
            self.error = True
            return
        ...
        metrics_to_calculate_ttm = ["Total Revenue", "Gross Profit", "Operating Income", "Net Income", "Operating Revenue"]
        ...
        if(len(quarterly_financials) > 0):
            ...
            for metric in metrics_to_calculate_ttm:
                ttm_value = financials[metric].iloc[0]
                ttm_data[metric] = ttm_value
        ...
        cfInfo.add_custom_info(info, financials)
